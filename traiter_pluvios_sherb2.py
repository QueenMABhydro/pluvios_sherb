...
def visualiser_grilles_csv(grille_krigee, radar_grid, donnees_pluvios, emplacements_pluvios, date_heure):
    ...
    # 1) S'assurer que l'echelle de couleur est la meme pour la grille et les points
    donnees_classees=donnees_classees.iloc[0,:].to_numpy(dtype='float') # l'option float 
    #est importante parce que sinon il enregistre les chiffres en tout ou en 
    #partie comme des strings et ca cree des erreurs dans la recherche des min et max
        
    min_obs=min(donnees_classees)
    max_obs=max(donnees_classees)
    min_grille=max(precip_reshape)
    max_grille=min(precip_reshape)
    ...
    plt.scatter(valeurs_pluvios_date_classe['X'], valeurs_pluvios_date_classe['Y'], c=valeurs_pluvios_date_classe['valeur'].astype(float), cmap='Blues', edgecolor='black',s=80, norm=norm)
